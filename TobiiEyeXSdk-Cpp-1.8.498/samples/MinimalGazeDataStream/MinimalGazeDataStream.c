...
void OnGazeDataEvent(TX_HANDLE hGazeDataBehavior)
{
	...
	if (txGetGazePointDataEventParams(hGazeDataBehavior, &eventParams) == TX_RESULT_OK) {
		printf("Gaze Data: (%08.1f, %08.1f) timestamp %.0f ms\n", eventParams.X, eventParams.Y, eventParams.Timestamp);
		char buffer[1024];
		sprintf_s(buffer, 18, "%08.1f,%08.1f", eventParams.X, eventParams.Y);
		if (sendto(s, buffer, strlen(buffer), 0, (struct sockaddr *) &si_other, slen) == SOCKET_ERROR)
		{
